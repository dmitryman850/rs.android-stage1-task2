    ...
    fun toTextFormat(hour: String, minute: String):String {
        ...
        var result = ""
         when (m) {
            ...
            in 2..14 -> result = "${time[m - 1]} minutes past ${time[h - 1]}"
            15 -> result = "quarter past ${time[h - 1]}"
            in 16..29 -> result = "${time[m - 1]} minutes past ${time[h - 1]}"
            30 -> result = "half past ${time[h - 1]}"
            31 -> result = "${time[m - 1]} minute to ${time[h - 1]}"
            in 32..44 -> result = "${time[59 - m]} minutes to ${time[h]}"
            45 -> result = "quarter to ${time[h]}"
            in 46..59 -> result = "${time[59 - m]} minutes to ${time[h]}"
        }
        println(result)
        return result
